...
public class SessionRulesServiceBase {
  ...
  public Optional<SessionRules> supplySessionRules(
    ...
    Optional<Rule> maybeRule = rules
      ...
      .filter(
        athleticLevelRule ->
          athleticLevelRule.getAthleticLevel() == profile.getAthleticLevel()
      )
      .findFirst();
    ...
    if (!maybeRule.isPresent()) {
      throw new RuntimeException(
        String.format(
          "unable to find rules for athleticLevel: %s",
          profile.getAthleticLevel()
        )
      );
    }
  ...
  private JsonNode loadJson(String filename) {
    ...
    try (
      InputStream fileInputStream = SessionRulesServiceBase.class.getResourceAsStream(
          resourceName
        )
    ) {
